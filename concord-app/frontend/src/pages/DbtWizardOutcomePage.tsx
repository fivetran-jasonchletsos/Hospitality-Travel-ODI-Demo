/*
 * DbtWizardOutcomePage — Post-build outcome page for the Ardmore Hotels dbt-wizard demo.
 *
 * Route: /outcome
 *
...
export default function DbtWizardOutcomePage() {
  const [o, setO] = useState<OutcomeData | null>(null);

  useEffect(() => {
    fetch(wizardDataUrl('outcome.json'))
    ...
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <header className="mb-10">
        ...
        <p className="mt-3 max-w-3xl leading-relaxed text-lg text-[var(--ink-muted)]">
          ...
          {o.build_seconds} seconds — the same window the CRO waited for an answer.
        </p>
      </header>
      ...
      <section
        ...
        style={{ borderLeft: '5px solid var(--brass)', background: 'rgba(201,161,73,0.04)' }}
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            <div className="eyebrow mb-1">Affected properties</div>
            <div className="font-mono text-sm font-semibold text-[var(--ink-strong)]">
              {o.root_cause.affected_cohort}
            </div>
          </div>
          <div>
            <div className="eyebrow mb-1">Parity-breach properties</div>
            <div className="font-serif text-3xl font-semibold text-[var(--brass)]">
          ...
          <div>
            ...
            <div className="font-mono text-sm font-semibold text-[var(--ink-strong)]">
              OTA channel · Tue-Wed bookings · 7-12% underpricing
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-10">
        <LineagePanel
          ...
          subtitle="No gold table tracks RevPAR attribution by brand, region, and week."
      ...
      <section className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
        ...
        <div className="rounded-lg border border-[var(--hairline)] bg-white p-5 shadow-sm"
          ...
          <div className="font-serif text-3xl font-semibold text-[var(--good)]">
      ...
      <section
        ...
        style={{ borderLeft: '5px solid var(--good)', background: 'rgba(22,101,52,0.04)' }}
      >
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 items-center">
          <div className="sm:col-span-1">
            <div className="eyebrow mb-2">dbt-wizard result</div>
            <div className="font-serif text-6xl sm:text-7xl font-semibold tracking-tight text-[var(--good)]">
              {o.hero.value}
            </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 rounded-lg border border-[var(--hairline)] bg-white p-5 items-center justify-between shadow-sm">
        ...
        <div className="flex gap-2 flex-wrap">
          <Link
            to="/"
            className="inline-flex items-center gap-2 rounded-md border border-[var(--hairline)] bg-white text-[var(--ink-strong)] font-semibold px-5 py-2.5 hover:bg-[var(--ivory-deep)] transition-colors"
          >
            Back to home
          </Link>
          <Link
...
function WizardColumn({ data, tone }: { data: Column; tone: 'crisis' | 'resolved' }) {
  const toneColor =
    tone === 'crisis' ? 'var(--brass)' : 'var(--good)';
  const chipClass =
    tone === 'crisis' ? 'status-pill warn' : 'status-pill good';
  return (
    <div
    ...
    >
      ...
      <dl className="space-y-2 my-5 rounded-md border border-[var(--hairline)] bg-[var(--ivory-deep)] p-4">
        {data.metrics.map(m => (
              ...
              {grouped[layer].map(n => {
                ...
                return (
                  ...
                  >
                    ...
                    {isGap && (
                      <div className="font-mono text-[10px] mt-1 text-[var(--bad)]">NOT BUILT</div>
                    )}
                    {isNew && (
