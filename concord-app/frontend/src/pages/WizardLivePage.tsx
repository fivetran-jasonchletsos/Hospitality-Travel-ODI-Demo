/*
 * WizardLivePage — dbt-wizard live-build playback for Ardmore Hotels.
 *
...
const STEP_DEFS = [
  ...
  { label: 'Schema Understanding', who: 'Summary',      tools: 'describe, lineage',       insight: '16 cols · 0 null keys' },
...
// Agent accent colors aligned with Ardmore Hotels palette
...
export default function WizardLivePage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(wizardDataUrl('agents.json')).then(r => { if (!r.ok) throw new Error(`Fetch failed: ${r.status}`); return r.json(); }),
      fetch(wizardDataUrl('scenario.json')).then(r => { if (!r.ok) throw new Error(`Fetch failed: ${r.status}`); return r.json(); }),
      fetch(wizardDataUrl('build_script.json')).then(r => { if (!r.ok) throw new Error(`Fetch failed: ${r.status}`); return r.json(); }),
    ]).then(([a, s, b]) => {
      setAgents(a.agents);
      setScenario(s);
      setEvents(b.events);
    ...
    <div className="wizard-terminal mx-auto max-w-[1640px] px-4 py-4 sm:px-6 lg:px-8">
      ...
      <div
        ...
        style={{
          ...
          boxShadow: '0 2px 8px rgba(13,59,56,0.08)',
        }}
      >
        <div className="flex items-center gap-3 flex-wrap">
          <span
            ...
            style={{
              ...
              background: 'rgba(13,148,136,0.1)', color: '#0d9488', border: '1px solid rgba(13,148,136,0.35)',
            }}
          >
            <span
              style={{
          ...
          <span className="font-mono" style={{ color: 'var(--ink-muted)', fontSize: 13 }}>
            Step{' '}
            <span style={{ color: '#0d9488', fontWeight: 700 }}>{currentStep}/{totalSteps}</span>
          ...
          >
            <div
              style={{
                ...
                height: '100%',
                background: complete ? 'var(--good)' : '#0d9488',
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            style={{ background: 'var(--ivory-deep)', borderColor: 'var(--hairline)', color: 'var(--ink)', padding: '7px 14px', fontSize: 13 }}
      ...
      >
        <div className="min-w-0 flex-shrink" style={{ flex: '1 1 460px' }}>
          <div className="eyebrow" style={{ fontSize: 10, marginBottom: 2, color: '#0d9488' }}>
            Revenue Management · {scenario.timezone_label} · {scenario.requested_by}
          </div>
          <p
        ...
        <div className="font-mono text-[var(--ink-muted)] shrink-0" style={{ fontSize: 11 }}>
          Target: <span style={{ color: '#0d9488', fontWeight: 700 }}>{scenario.metric_code}</span>
        </div>
      </div>
            ...
            >
              <div
                ...
                style={{
                  ...
                  color: active ? '#0d9488' : done ? 'var(--good)' : 'var(--ink-soft)',
                }}
              >
        ...
        >
          ...
          >
            {visibleNarr.map((m, idx) => {
              ...
              const color = a?.color ?? AGENT_STEP_COLOR[m.e.from] ?? '#0d9488';
        ...
        <section className="flex flex-col gap-3 lg:!h-[calc(100dvh-440px)]" style={{ minHeight: 'max(60vh, 300px)' }}>
            ...
            >
              <div className="flex items-center gap-3 flex-wrap min-w-0">
                <div className="eyebrow font-mono" style={{ fontSize: 11, letterSpacing: '0.02em' }}>
                  models/gold/fct_revpar_attribution_by_brand_region_week.sql
                </div>
                <span
          ...
          <div className="research-card flex flex-col" style={{ flex: '1 1 0' }}>
            ...
            >
              <div className="flex items-center gap-3 flex-wrap min-w-0">
                <div className="eyebrow font-mono" style={{ fontSize: 11, letterSpacing: '0.02em' }}>
                  models/gold/fct_revpar_attribution_by_brand_region_week.yml
                </div>
                <span
                  className="layer-chip"
                  style={{
              ...
              >
                <span
                  style={{
                    ...
                    background: i === 0 ? '#0d9488' : 'var(--ink-soft)',
            ...
            <div className="flex items-baseline gap-3 flex-wrap">
              <div
                ...
                style={{
                  ...
                  background: 'rgba(22,101,52,0.12)', color: 'var(--good)',
      ...
      <style>{`
        ...
        .wizard-terminal::after {
          content: 'ardmore-hotels/wizard-live · dbt-wizard';
          position: absolute;
          top: 0; left: 0; right: 0;
          height: 28px;
          display: flex;
        ...
        .wizard-terminal [style*="color: #0d9488"] { color: var(--t-accent) !important; }
        ...
        .wizard-terminal .research-card[style*="rgba(13,148,136"] {
          background: rgba(45,212,191,0.10) !important;
        }
        .wizard-terminal .research-card[style*="rgba(22,101,52"] {
          background: rgba(74,222,128,0.10) !important;
        }
        .wizard-terminal .research-card[style*="var(--ivory-deep)"] {
          background: var(--t-surface) !important;
        }
        .wizard-terminal pre {
          background: var(--t-bg) !important;
        ...
        .wizard-terminal div[style*="background: var(--ivory-deep)"] {
          background: var(--t-elev) !important;
          border-color: var(--t-line) !important;
        }
...
const SQL_KEYWORDS = new Set([
  ...
  'sum', 'max', 'min', 'avg', 'coalesce', 'date_trunc', 'dayofweek', 'is', 'not',
]);
